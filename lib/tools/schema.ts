...
export const TOOLS = {
  show_section: tool(
    "Scroll to and expand a section of the page.",
    z.object({ section: z.enum(SECTIONS) }),
  ),
  show_project: tool(
    "Expand a project card inline while talking about it.",
    z.object({ slug: z.enum(PROJECT_SLUGS) }),
  ),
  highlight_proof_point: tool(
    "Highlight one proof point card by id (pp-01 … pp-12).",
    z.object({ id: z.string().regex(/^pp-\d{2}-[a-z-]+$/) }),
  ),
  show_availability: tool(
    "Show the availability and logistics card (markets, timezone overlap, engagement type).",
    z.object({}),
  ),
  switch_language: tool(
    "Switch the page locale and transcript direction. In voice, the agent's own language_detection tool handles audio.",
  ...
  open_leave_message: tool(
    "Open the leave-a-message form; the visitor types and submits it themselves.",
    z.object({}),
  ),
  open_email_summary: tool(
    "Offer to e-mail the visitor a summary; the visitor enters their address and consents themselves. Once per session.",
    z.object({}),
  ),
  offer_lead_capture: tool(
    "Offer, once per session and only after the visitor expresses hiring intent, a small card to leave name, company and e-mail. Never pressure.",
    z.object({ reason: z.string().max(120) }),
    true,
  ),
  show_contact: tool(
    "Show the contact card: hello@abdelrahmansheta.com, LinkedIn, the message form and the booking link.",
    z.object({}),
  ),
} as const;
